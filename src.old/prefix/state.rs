...
/// SQLite-backed cache for Wine prefix data.
///
/// Stores only program-derived data (scan results, registry hot-cache).
/// User-editable data lives in `tequila-config.json` and Wine `.reg` files.
...
impl PrefixStore {
    ...
    /// Returns all (key, value) pairs for a registry section.
    ...
    /// Check if any registry cache exists for a prefix.
    pub fn has_registry_cache(&self, prefix_path: &str) -> bool {
        let db = self.db.lock().unwrap();
        let mut stmt = db.prepare(
            "SELECT COUNT(*) FROM registry_settings WHERE prefix_path = ?1"
        ).ok();

        stmt.as_mut()
            .and_then(|s| s.query_row(params![prefix_path], |row| row.get::<_, i64>(0)).ok())
            .map(|count| count > 0)
            .unwrap_or(false)
    }

    /// Check if scanned executables cache exists for a prefix.
    pub fn has_scanned_prefix(&self, prefix_path: &str) -> bool {
        let db = self.db.lock().unwrap();
        db.prepare("SELECT 1 FROM scanned_executables WHERE prefix_path = ?1 LIMIT 1")
            .ok()
            .and_then(|mut s| s.query_row(params![prefix_path], |_| Ok(())).ok())
            .is_some()
    }
    ...
    /// Invalidate all registry cache for a prefix (e.g., after external modification).
    ...
    // ── Scanned Executables (scan cache) ──
    ...
    pub fn list_scanned_executables(&self, prefix_path: &str) -> Result<Vec<RegisteredExecutable>> {
        ...
        let exes: Vec<RegisteredExecutable> = stmt.query_map(params![prefix_path], |row| {
            Ok(RegisteredExecutable {
                name: row.get("name")?,
                executable_path: std::path::PathBuf::from(row.get::<_, String>("executable_path")?),
                description: row.get("description")?,
                icon_path: row.get::<_, Option<String>>("icon_path")?.map(std::path::PathBuf::from),
