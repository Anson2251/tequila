...
impl ProcessTracker {
    pub fn new() -> Self { Self { running: HashMap::new() } }
    ...
    pub fn poll_dead(&mut self) -> Vec<PathBuf> {
        ...
        self.running.retain(|path, children| {
            children.retain_mut(|child| match child.try_wait() {
                Ok(Some(_)) => { changed.push(path.clone()); false }
                Ok(None) => true,
                Err(_) => { changed.push(path.clone()); false }
            });
            !children.is_empty()
        });
        changed.sort();
    ...
    pub fn is_running(&self, exe_path: &Path) -> bool { self.running.contains_key(exe_path) }

    pub fn running_paths(&self) -> Vec<PathBuf> { self.running.keys().cloned().collect() }
