...
impl PrefixStore {
    ...
    pub fn has_registry_cache(&self, prefix_path: &str) -> bool {
        let db = self.db.lock().unwrap();
        let mut stmt = match db.prepare("SELECT COUNT(*) FROM registry_settings WHERE prefix_path = ?1") {
            Ok(s) => s,
            Err(_) => return false,
        };
    ...
    pub fn has_scanned_prefix(&self, prefix_path: &str) -> bool {
        let db = self.db.lock().unwrap();
        let mut stmt = match db.prepare("SELECT 1 FROM scanned_executables WHERE prefix_path = ?1 LIMIT 1") {
            Ok(s) => s,
            Err(_) => return false,
        };
    ...
    pub fn invalidate_registry_cache(&self, prefix_path: &str) -> Result<()> {
        let db = self.db.lock().unwrap();
        db.execute("DELETE FROM registry_settings WHERE prefix_path = ?1", params![prefix_path]).map_err(map_err)?;
        Ok(())
    }

    pub fn save_scanned_executables(&self, prefix_path: &str, exes: &[RegisteredExecutable]) -> Result<()> {
        let db = self.db.lock().unwrap();
        db.execute("DELETE FROM scanned_executables WHERE prefix_path = ?1", params![prefix_path]).map_err(map_err)?;
        if exes.is_empty() { return Ok(()); }
        let mut stmt = db.prepare(
        ...
        for exe in exes {
            stmt.execute(params![
                prefix_path, exe.executable_path.to_string_lossy().to_string(), exe.name,
                exe.description, exe.icon_path.as_ref().map(|p| p.to_string_lossy().to_string()),
                exe.file_version, exe.product_version, exe.company_name,
                exe.file_description, exe.product_name,
            ]).map_err(map_err)?;
        }
        Ok(())
    }
    ...
    pub fn list_scanned_executables(&self, prefix_path: &str) -> Result<Vec<RegisteredExecutable>> {
        ...
        let exes = stmt.query_map(params![prefix_path], |row| {
