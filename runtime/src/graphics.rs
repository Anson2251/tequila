...
pub fn graphics_dir() -> PathBuf {
    dirs::data_dir()
        ...
        .join("tequila").join("graphics")
}

pub fn install_symlink(src: &Path, target: &Path) -> Result<()> {
    if let Some(parent) = target.parent() { fs::create_dir_all(parent)?; }
    match target.symlink_metadata() {
        Ok(m) if m.file_type().is_symlink() => { fs::remove_file(target)?; }
        Ok(_) => { let backup = target.with_extension("old"); fs::rename(target, &backup)?; }
        Err(_) => {}
    }
...
pub fn remove_symlinks(dir: &Path, filenames: &[&str]) -> Result<()> {
    for name in filenames {
        ...
        if let Ok(m) = target.symlink_metadata() {
            if m.file_type().is_symlink() { fs::remove_file(&target)?; }
        }
    }
    Ok(())
}
...
pub async fn download_dxmt(version: &str, download_url: &str, progress: &download::ProgressFn) -> Result<PathBuf> {
...
fn find_dxmt_so_dir(dxmt_dir: &Path) -> Option<PathBuf> {
    let candidates = [dxmt_dir.join("lib").join("wine").join("x86_64-unix"), dxmt_dir.join("x86_64-unix")];
    candidates.into_iter().find(|d| d.is_dir())
}

fn find_dxmt_dll_dir(dxmt_dir: &Path) -> Option<PathBuf> {
    let candidates = [dxmt_dir.join("lib").join("wine").join("x86_64-windows"), dxmt_dir.join("x86_64-windows")];
    candidates.into_iter().find(|d| d.is_dir())
}
...
pub fn activate_dxmt_for_runtime(dxmt_dir: &Path, runtime: &Runtime) -> Result<()> {
    ...
    let target_dir = runtime.bundle_dir.join("lib").join("wine").join("x86_64-unix");
    fs::create_dir_all(&target_dir)?;
    for entry in fs::read_dir(&so_src)? {
        let entry = entry?;
        let fname = entry.file_name();
        if fname.to_string_lossy().ends_with(".so") { install_symlink(&entry.path(), &target_dir.join(&fname))?; }
    }
    Ok(())
}
...
pub fn activate_dxmt_for_prefix(dxmt_dir: &Path, prefix_path: &Path) -> Result<()> {
    ...
    for name in DXMT_DLLS { let src = dll_src.join(name); if src.exists() { install_symlink(&src, &system32.join(name))?; } }
    Ok(())
}
...
pub fn extract_d3dmetal_from_dmg(dmg_path: &Path, version: &str) -> Result<PathBuf> {
    ...
    let output = Command::new("hdiutil")
        .arg("attach").arg(dmg_path).arg("-mountpoint").arg(&base_mount).arg("-nobrowse")
        .output()
        .map_err(|e| PrefixError::Process(format!("Failed to run hdiutil: {}", e)))?;
    if !output.status.success() {
        return Err(PrefixError::Process(format!("hdiutil attach failed: {}", String::from_utf8_lossy(&output.stderr))));
    }
    let external_src = find_external_in_dmg(&base_mount);
    let result = match external_src {
        Some(src) => {
            fs::create_dir_all(&dest_dir)?;
            let status = Command::new("cp").arg("-r").arg(&src).arg(dest_dir.to_string_lossy().as_ref())
                .status().map_err(|e| PrefixError::Process(format!("Failed to copy files: {}", e)))?;
            if !status.success() { Err(PrefixError::Process("Failed to copy D3DMetal files".to_string())) }
            else { Ok(dest_dir.clone()) }
        }
        None => Err(PrefixError::NotFound("Could not find lib/external or redist/lib/external in DMG".to_string())),
    };
...
fn find_external_in_dmg(mount: &Path) -> Option<PathBuf> {
    ...
    if redist.is_dir() { return Some(redist); }
    if let Some(inner_dmg) = find_inner_dmg(mount) {
        ...
        if std::fs::create_dir(&inner_mount).is_ok() {
            let ok = Command::new("hdiutil")
                .arg("attach").arg(&inner_dmg).arg("-mountpoint").arg(&inner_mount).arg("-nobrowse")
            ...
            if ok {
                ...
                let result = if inner_external.is_dir() { Some(inner_external) } else if inner_direct.is_dir() { Some(inner_direct) } else { None };
...
fn find_inner_dmg(mount: &Path) -> Option<PathBuf> {
    walkdir::WalkDir::new(mount).max_depth(2).into_iter().flatten()
        .find(|e| e.file_name().to_str().map(|n| n.ends_with(".dmg")).unwrap_or(false))
...
pub fn activate_d3dmetal_for_runtime(d3dmetal_dir: &Path, runtime: &Runtime) -> Result<()> {
    ...
    for entry in walkdir::WalkDir::new(d3dmetal_dir).max_depth(4).into_iter().flatten() {
        let path = entry.path();
        let fname = path.file_name().and_then(|n| n.to_str()).unwrap_or("");
        if fname.ends_with(".so") || fname.ends_with(".framework") {
            let rel = path.strip_prefix(d3dmetal_dir).unwrap();
            install_symlink(path, &runtime_lib.join(rel))?;
        }
    }
    Ok(())
}
...
const DXVK_DLLS: &[&str] = &["d3d8.dll", "d3d9.dll", "d3d10core.dll", "d3d11.dll", "dxgi.dll"];
...
pub async fn fetch_vkd3d_release() -> Result<(String, String)> {
    ...
    let asset = release.assets.iter()
        ...
        .ok_or_else(|| PrefixError::NotFound("No tar.zst asset in VKD3D-Proton release".to_string()))?;
    Ok((release.tag_name.clone(), asset.browser_download_url.clone()))
}

pub async fn download_linux_backend(name: &str, version: &str, download_url: &str, is_zst: bool, progress: &download::ProgressFn) -> Result<PathBuf> {
    let gfx_dir = graphics_dir();
    ...
    download::download_file(download_url, &archive_path, progress).await?;
    if is_zst { download::extract_tar_zst(&archive_path, &tmp_dir)?; }
    else { download::extract_tar(&archive_path, &tmp_dir)?; }
    let content_dir = find_content_dir(&tmp_dir)?;
    fs::rename(&content_dir, &dest_dir)?;
    let _ = fs::remove_dir_all(&tmp_dir);
    Ok(dest_dir)
...
pub fn activate_for_prefix(backend: &base::GraphicsBackend, runtime: &Runtime, prefix_path: &Path) -> Result<GraphicsConfig> {
    let gfx_dir = graphics_dir();
    match backend {
        base::GraphicsBackend::Dxmt { version } => {
            let dxmt_dir = gfx_dir.join(format!("dxmt-{}", version));
            if !dxmt_dir.exists() { return Err(PrefixError::NotFound(format!("DXMT {} not installed", version))); }
            activate_dxmt_for_runtime(&dxmt_dir, runtime)?;
            activate_dxmt_for_prefix(&dxmt_dir, prefix_path)?;
            Ok(GraphicsConfig { backend: "dxmt".to_string(), version: version.clone() })
        }
        base::GraphicsBackend::D3DMetal { version } => {
            let d3dmetal_dir = gfx_dir.join(format!("d3dmetal-{}", version));
            if !d3dmetal_dir.exists() { return Err(PrefixError::NotFound(format!("D3DMetal {} not installed", version))); }
            activate_d3dmetal_for_runtime(&d3dmetal_dir, runtime)?;
            activate_d3dmetal_for_prefix(&d3dmetal_dir, prefix_path)?;
            Ok(GraphicsConfig { backend: "d3dmetal".to_string(), version: version.clone() })
        }
        base::GraphicsBackend::DxvkVkd3d { dxvk_version, vkd3d_version } => {
            ...
            if !dxvk_dir.exists() || !vkd3d_dir.exists() {
                return Err(PrefixError::NotFound(format!("DXVK {} or VKD3D {} not installed", dxvk_version, vkd3d_version)));
            }
            ...
            Ok(GraphicsConfig { backend: "dxvk-vkd3d".to_string(), version: format!("dxvk-{}+vkd3d-{}", dxvk_version, vkd3d_version) })
        }
    }
}
...
pub fn deactivate_for_prefix(config: &GraphicsConfig, prefix_path: &Path) -> Result<()> {
    match config.backend.as_str() {
            ...
            if let Ok(entries) = fs::read_dir(&system32) {
                for entry in entries.flatten() {
                    ...
                    if let Ok(m) = path.symlink_metadata() {
                        if m.file_type().is_symlink() && (fname.contains("D3DMetal") || fname.contains("xremetal") || fname.contains("nvngx")) {
        ...
        "dxvk-vkd3d" => {
            ...
            for dll in DXVK_DLLS { for dir in [&system32, &syswow64] { if let Ok(m) = dir.join(dll).symlink_metadata() { if m.file_type().is_symlink() { let _ = fs::remove_file(dir.join(dll)); } } } }
            for dll in VKD3D_DLLS { for dir in [&system32, &syswow64] { if let Ok(m) = dir.join(dll).symlink_metadata() { if m.file_type().is_symlink() { let _ = fs::remove_file(dir.join(dll)); } } } }
            Ok(())
        }
        _ => Ok(()),
    }
...
fn activate_dxvk_for_prefix(dxvk_dir: &Path, prefix_path: &Path) -> Result<()> {
    ...
    for arch_dir in ["x64", "x32"] {
        ...
        for dll in DXVK_DLLS { let src = src_dir.join(dll); if src.exists() { install_symlink(&src, &target.join(dll))?; } }
    }
    Ok(())
}
...
fn activate_vkd3d_for_runtime(vkd3d_dir: &Path, runtime: &Runtime) -> Result<()> {
    ...
    for entry in walkdir::WalkDir::new(vkd3d_dir).max_depth(3).into_iter().flatten() {
        ...
        if fname.ends_with(".so") { install_symlink(entry.path(), &runtime_lib.join(entry.file_name()))?; }
    }
    Ok(())
}
