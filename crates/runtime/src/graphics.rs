...
pub fn installed_backends() -> Vec<GraphicsBackend> {
    ...
    #[cfg(target_os = "linux")]
    {
        let mut dxvk_ver: Option<String> = None;
        let mut vkd3d_ver: Option<String> = None;
        if let Ok(entries) = fs::read_dir(&dir) {
            for entry in entries.flatten() {
                let name = entry.file_name().to_string_lossy().to_string();
                if entry.file_type().map(|t| t.is_dir()).unwrap_or(false) {
                    if let Some(ver) = name.strip_prefix("dxvk-") {
                        dxvk_ver = Some(ver.to_string());
                    } else if let Some(ver) = name.strip_prefix("vkd3d-") {
                        vkd3d_ver = Some(ver.to_string());
                    }
                }
            }
        }
        if let (Some(dxvk), Some(vkd3d)) = (dxvk_ver, vkd3d_ver) {
