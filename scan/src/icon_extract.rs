use exe::VecPE;
use exe::types::{ResourceDirectory, ResolvedDirectoryID, ResourceID};
...
fn build_ico_from_group(
    ...
    struct IconPart { width: u8, height: u8, color_count: u8, planes: u16, bit_count: u16, data: Vec<u8> }
    ...
    for entry in group.entries.iter() {
        ...
        let matches = resource_dir.filter(Some(ResolvedDirectoryID::ID(ResourceID::Icon as u32)), Some(id), None);
        let data_entry = matches.first()?.get_data_entry(image).ok()?;
        let raw = data_entry.read(image).ok()?;
        parts.push(IconPart {
            width: entry.width, height: entry.height, color_count: entry.color_count,
            planes: entry.planes, bit_count: entry.bit_count, data: raw.to_vec(),
        });
    }
    ...
    for part in &parts { buf.extend_from_slice(&part.data); }
    Some(buf)
}
