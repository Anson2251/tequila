...
impl IconCache {
    pub fn open(cache_dir: PathBuf) -> Result<Self, String> {
        ...
        if needs_migration {
            ...
            for entry in std::fs::read_dir(&cache_dir).into_iter().flatten().flatten() {
                ...
                if ext == Some("ico") || ext == Some("png") { std::fs::remove_file(path).ok(); }
            }
        }
        ...
        Ok(Self { db: Mutex::new(db), cache_dir })
    }
    ...
    pub fn put(&self, sha256: &str, png_data: &[u8]) -> Result<(), String> {
        ...
        db.execute("INSERT OR REPLACE INTO icons (sha256, icon_blob) VALUES (?1, ?2)", params![sha256, png_data])
    ...
    pub fn icon_path(&self, sha256: &str) -> Option<PathBuf> {
        ...
        if png_path.exists() {
            if png_path.metadata().ok().map(|m| m.len()).unwrap_or(0) > 0 { return Some(png_path); }
